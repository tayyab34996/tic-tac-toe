...
int main() {
    ...
    //sprite is an object that represents an image
    sf::Text startText("Start", font);
    startText.setCharacterSize(70);
    startText.setFillColor(sf::Color::White);
    startText.setPosition(830, 570);
    //creates start texr ,setsposition ,sets its color,size,and font
    ...
    while (startWindow.isOpen()) {
        ...
        while (startWindow.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonReleased) {
                sf::Vector2f mousePos = sf::Vector2f(sf::Mouse::getPosition(startWindow));//get the position of mouse when button is clicked
                if (startText.getGlobalBounds().contains(mousePos)) {//checks if the position was inside the botton of start text
                    startWindow.close();//if true our  intro window is close

                    sf::RenderWindow window(sf::VideoMode(1080, 780), "Tic-Tac-Toe Level 1");//since our window is closed here we created 
                    //a new window and this pattern is followed for level 2 and 3 as well

                    sf::Texture backgroundTexture;
                    if (!backgroundTexture.loadFromFile("2 (1).jpg")) {
                        std::cerr << "Failed to load background image." << std::endl;
                        return 1;
                    }//loads  background texture for new window
                    sf::Sprite backgroundSprite(backgroundTexture);

                    const float buttonSize = 90.f;
                    const float padding = 25.f;//padding is actuall space betweeen buttons
                    const int gridSize = 3;

                    float gridWidth = gridSize * (buttonSize + padding) - padding;//if padding is high then there will be more space 
                    //betweeen the boxes in the grid
                    float gridHeight = gridSize * (buttonSize + padding) - padding;
                    float posX = (window.getSize().x - gridWidth) / 2;
                    float posY = (window.getSize().y - gridHeight) / 2;//gets the position of x and y axis related to our openend window

                    sf::RectangleShape buttons[gridSize][gridSize];//now we created  array buttons in the grid 
                    sf::Text buttonTexts[gridSize][gridSize];// and array of texts 
                    ...
                    //for level 1  created text which is displayed in the center  
                    sf::Text winnerText;
                    ...
                    //local bounds are used to get dimension of text as well as its position starting from top left corner
                    for (int i = 0; i < gridSize; ++i) {
                        for (int j = 0; j < gridSize; ++j) {
                            buttons[i][j].setPosition(posX + j * (buttonSize + padding), posY + i * (buttonSize + padding));//sets teh position of easch button
                            buttons[i][j].setSize(sf::Vector2f(buttonSize, buttonSize));
                            buttons[i][j].setFillColor(sf::Color::Transparent);//inner  color is transparent 
                            buttons[i][j].setOutlineThickness(7);// it is the thickness of white linewhich is actually our button
                            buttons[i][j].setOutlineColor(sf::Color::White);//color of outline is white
                            ...
                            //depending on the term of players related text will be pasted  at this position
                        }
                    }
                    ...
                    while (window.isOpen()) {
                        ...
                        while (window.pollEvent(event)) {
                            ...
                            if (event.type == sf::Event::MouseButtonPressed) {
                                if (event.mouseButton.button == sf::Mouse::Left && !winner) {//checks if the mouse button pressed  is left or  not and 
                                    //if the winner is not found yet
                                    sf::Vector2f mousePos = window.mapPixelToCoords(sf::Mouse::getPosition(window));//it gets the position of the mouse
                                    //according to world coordinates
                                    for (int i = 0; i < gridSize; ++i) {
                                        for (int j = 0; j < gridSize; ++j) {
                                            if (buttons[i][j].getGlobalBounds().contains(mousePos) && matrix[i][j] == 0) {//if we have position greater than zero 
                                                //ans the index is currently empty (means no value placed yet)
                                                if (player_1 == 1) {
                                                    matrix[i][j] = 1;
                                                    buttonTexts[i][j].setString("X");
                                                    if (CheckWinner(matrix, size)) {//function called
                                                        winner = true;
                                                        winnerText.setString("Player 1 wins! (X)");
                                                    }
                                                    player_2 = 1;//changes for 2nd players turn 
                                                    player_1 = 0;
                                                }
                                                else if (player_2 == 1) {
                                                    matrix[i][j] = 2;
                                                    ...
                                                    player_1 = 1;
                                                    player_2 = 0;//angain changes ofr 1st player
                                                }
                                            }
                                        }
                                    }
                                ...
                                if (event.mouseButton.button == sf::Mouse::Left) {
                                    ...
                                    if (resetText.getGlobalBounds().contains(mousePos)) {//if mouse is in the bounds of reset text button then it will 
                                        //then it will assign all of the indexes to null character
                                    ...
                                    if (leveltext.getGlobalBounds().contains(mousePos)) {
                                        window.close();//if mouse is in the bounds of levle button on top right corner then window will close 
                                        ...
                                        //next few lines are repeated concept of drawing andloading files for levle 2
                                        sf::Font font;
                                        if (!font.loadFromFile("ShortBaby-Mg2w.ttf")) {
                                            std::cerr << "Failed to load font." << std::endl;
                                            return 1;
                                        ...
                                        for (int i = 0; i < gridSize; ++i) {
                                            for (int j = 0; j < gridSize; ++j) {//same as above explained
                                            ...
                                            while (window.pollEvent(event)) {
                                                ...
                                                if (event.type == sf::Event::MouseButtonPressed) {
                                                    if (event.mouseButton.button == sf::Mouse::Left && !winner) {
                                                        ...
                                                        for (int i = 0; i < gridSize; ++i) {
                                                            for (int j = 0; j < gridSize; ++j) {
                                                                if (buttons[i][j].getGlobalBounds().contains(mousePos) && matrix[i][j] == 0) {//same way to get positoin
                                                                    if (p1 == 1) {
                                                                        matrix[i][j] = 1;
                                                                        buttonTexts[i][j].setString("X");
                                                    ...
                                                    if (event.mouseButton.button == sf::Mouse::Left) {//again for reset assign to null charaacter
                                                        sf::Vector2f mousePos = window.mapPixelToCoords(sf::Mouse::getPosition(window));
                                                        ...
                                                        if (event.type == sf::Event::MouseButtonPressed) {
                                                            ...
                                                            if (event.mouseButton.button == sf::Mouse::Left && leveltext.getGlobalBounds().contains(mousePos)) {
                                                                ...
                                                                while (window.isOpen()) {
                                                                    ...
                                                                    //Did this for eevery level in the rest of the code
